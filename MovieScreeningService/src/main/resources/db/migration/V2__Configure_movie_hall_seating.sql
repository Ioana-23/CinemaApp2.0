CREATE
OR REPLACE FUNCTION generate_uuid()
    RETURNS INT
    LANGUAGE plpgsql
AS
$$
...
BEGIN
SELECT
INTO nr_cifre floor(random() * (6-4+1) + 4)::int;
FOR index IN 1..nr_cifre LOOP
SELECT
INTO cifra_curenta floor(random() * 10 + 1)::int;
uuid_final
:= uuid_final * 10 + cifra_curenta;
END LOOP;
RETURN uuid_final;
...
INT;
BEGIN
INSERT INTO movie_halls
VALUES (1, 1);

SELECT
INTO nr_linii floor(random() * (nr_maxim_linii-nr_minim_linii+1) + nr_minim_linii)::int;
SELECT
INTO nr_coloane floor(random() * (nr_maxim_coloane-nr_minim_coloane+1) + nr_minim_coloane)::int;
...
END LOOP;
INSERT INTO movie_halls
VALUES (movie_hall, movie_hall_uuid);
...
END LOOP;
INSERT INTO seats
VALUES (DEFAULT, i, j, movie_hall, seat_uuid);
END LOOP;
END LOOP;
END LOOP;
END;
